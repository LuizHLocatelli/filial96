      ...
      {isSearching ? (
        ...
        <div className="p-2">
          ...
          {searchResults.map((result) => {
            ...
            const isEmoji = !IconComponent && result.icon && result.icon.length <= 4;
              ...
              >
                <div className="flex items-start gap-3 w-full">
                  {IconComponent ? (
                    ...
                    <span className="text-sm mt-0.5 flex-shrink-0 w-4 text-center">{result.icon}</span>
                  ) : (
                    <List className="h-4 w-4 mt-0.5 text-muted-foreground flex-shrink-0" />
                  )}
