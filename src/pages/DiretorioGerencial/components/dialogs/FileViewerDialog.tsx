...
export function FileViewerDialog({
  ...
  const handleSend = async () => {
    if (!input.trim() || aiChat.isPending || !arquivo) return;
        ...
        onError: () => {
          setMessages((prev) => [
            ...prev,
            {
              ...
              parts: [
                {
                  text: "Desculpe, ocorreu um erro ao analisar sua pergunta. Tente novamente mais tarde.",
                },
              ],
            },
          ]);
  ...
  const renderMessageContent = (content: string) => {
    const html = content
      ...
      .replace(
        /```(.*?)```/gs,
        '<pre className="bg-muted p-2 rounded-md overflow-x-auto"><code>$1</code></pre>'
      )
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className="max-w-6xl max-h-[75dvh] sm:max-h-[75vh] overflow-hidden flex flex-col p-0 glass-dialog"
        hideCloseButton
      >
            ...
            {isImage ? (
              ...
              <div className="flex flex-col items-center text-muted-foreground p-8 text-center">
                ...
                <p className="mt-2 text-sm max-w-sm">
                  O visualizador nativo não suporta este formato de arquivo. Faça o
                  download para visualizar o conteúdo.
                </p>
                <Button
                  className="mt-6"
                ...
                <div className="flex flex-col items-center justify-center h-full text-center space-y-4 opacity-70">
                  ...
                  <div className="max-w-[250px]">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Ex: "Resuma os pontos principais", "Qual o valor total?"
                    </p>
                  </div>
                </div>
              ) : (
