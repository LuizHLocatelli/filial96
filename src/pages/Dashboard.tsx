...
import { BarChart3, Users, Calendar, TrendingUp, ArrowRight, CheckCircle2, Clock, AlertCircle, Activity, Target, Zap } from "lucide-react";
...
export default function Dashboard() {
  ...
  const fetchDashboardStats = async () => {
    try {
      const [tasksData, clientsData, depositsData, salesData] = await Promise.all([supabase.from("moveis_tarefas").select("status"), supabase.from("crediario_clientes").select("id"), supabase.from("crediario_depositos").select("id"), supabase.from("venda_o_sales").select("id")]);
  ...
  return <div className="space-y-6 max-w-7xl mx-auto animate-fade-in">
      {/* Header */}
      <div className="space-y-2 px-0 py-0 my-0">
        <h1 className={`font-bold tracking-tight gradient-text ${isMobile ? 'text-2xl' : 'text-3xl'}`}>Painel</h1>
        
      </div>

      {/* Statistics Cards */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 py-0 my-[8px]">
      ...
      <Tabs defaultValue="overview" className="space-y-4">
              ...
              <CardContent className="space-y-4">
                <div className="space-y-3">
                  ...
                  <div className="w-full bg-secondary rounded-full h-3 overflow-hidden">
                    <div className="bg-gradient-to-r from-primary to-primary/80 h-3 rounded-full transition-all duration-700 ease-out" style={{
            ...
            <Card className="glass-card">
              ...
              <CardContent className="space-y-4">
                <div className="space-y-3">
                  ...
                  status: "Ativo"
                }].map(system => <div key={system.name} className="flex justify-between items-center p-3 rounded-lg bg-green-50 dark:bg-green-950/10 border border-green-200 dark:border-green-800">
        ...
        <TabsContent value="actions" className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {quickActions.map((action, index) => <Card key={index} className="cursor-pointer transition-all duration-300 hover:shadow-strong hover:-translate-y-2 group" onClick={action.action}>
        ...
        <TabsContent value="recent" className="space-y-4">
          <Card className="glass-card">
            ...
            <CardContent>
              <div className="space-y-4">
                {[1, 2, 3].map(item => <div key={item} className="flex items-center gap-4 p-4 rounded-lg border border-border/50 hover:border-primary/30 transition-colors">
