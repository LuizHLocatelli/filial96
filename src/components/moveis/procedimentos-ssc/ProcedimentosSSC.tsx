...
export function ProcedimentosSSC() {
  ...
  return (
    <div className="w-full mx-auto space-y-4 sm:space-y-6 px-3 sm:px-4 lg:px-6 py-3 sm:py-4 lg:py-5 max-w-7xl">
      {/* Header */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Guia de assistência técnica por fabricante
          </p>
        </div>
        ...
        {filteredProcedimentos.length === 0 ? (
          ...
          filteredProcedimentos.map((proc, index) => (
            <motion.div
              ...
              animate={shouldReduceMotion ? { opacity: 1 } : { opacity: 1, y: 0 }}
              transition={shouldReduceMotion ? { duration: 0 } : { duration: 0.3, ease: "easeOut", delay: index * 0.03 }}
            >
      ...
      <Card className="bg-muted/50">
        <CardContent className="py-4">
          <div className="text-sm text-muted-foreground space-y-2">
            <p className="font-medium">Informações sobre o ZEEV:</p>
            <p>
