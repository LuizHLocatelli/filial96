...
- **Known Issue**: `supabase_execute_sql` returns `ReferenceError: crypto is not defined` due to a bug in `@supabase/mcp-server-supabase`. Workarounds:
  ...
  - For data operations: The `sql-proxy` Edge Function is available at `/functions/v1/sql-proxy` but requires HTTP access (not available via MCP directly). Report this bug to Supabase.
- **Edge Functions**: Located in `supabase/functions/` (Deno runtime). By default, they require JWT verification.
  - `sql-proxy`: Executes arbitrary SQL queries via HTTP POST. Endpoint: `POST /functions/v1/sql-proxy` with body `{"query": "SELECT ..."}`
