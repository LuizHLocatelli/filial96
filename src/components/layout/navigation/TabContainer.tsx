import { memo } from "react";
import { motion } from "framer-motion";
import { cn } from "@/lib/utils";
import { TabContainerProps } from "./types";

export const TabContainer = memo(function TabContainer({ children, isMobile, isSmallScreen }: TabContainerProps) {
  return (
    <motion.div 
      initial={{ y: 100, opacity: 0 }}
      animate={{ y: 0, opacity: 1 }}
      transition={{ duration: 0.5, ease: "easeOut" }}
      className={cn(
        "fixed z-50",
        isMobile 
          ? "bottom-4 left-4 right-4 flex justify-center"
          : "bottom-2 left-0 right-0 flex justify-center"
      )}
    >
      {/* Glassmorphism Container */}
      <div 
        role="tablist"
        aria-label="Navegação principal"
        className={cn(
          "relative overflow-visible glass-nav glass-primary rounded-3xl shadow-2xl",
          isMobile 
            ? cn(
                "w-full max-w-lg backdrop-blur-2xl",
                isSmallScreen ? "px-2 py-3" : "px-3 py-3"
              )
            : "rounded-3xl px-4 py-3.5 w-auto max-w-md mx-auto backdrop-blur-2xl"
        )}
      >
        
        {/* Animated Gradient Background */}
        <div className="absolute inset-0 opacity-80 rounded-3xl pointer-events-none">
          <div className="absolute inset-0 bg-gradient-to-r from-primary/20 via-emerald-500/10 to-green-500/15 rounded-3xl animate-pulse" />
          <div className="absolute inset-0 bg-gradient-to-b from-transparent via-primary/8 to-transparent rounded-3xl" />
        </div>

        {/* Glow Effect */}
        <div className="absolute inset-0 rounded-3xl bg-gradient-to-r from-primary/30 via-emerald-500/20 to-green-500/25 blur-xl opacity-50 -z-10 pointer-events-none" />
        
        {/* Content */}
        <div className="relative z-10">
          {children}
        </div>

        {/* Top Highlight */}
        <div className="absolute top-0 left-4 right-4 h-px bg-gradient-to-r from-transparent via-white/30 to-transparent rounded-full pointer-events-none" />
      </div>
    </motion.div>
  );
});

